...
repository and the shared `AGameEmpowerment/Developer-Toolbox` repository. Keep
...
4. When no checkout exists, stop Toolbox-dependent work and ask the user to
   clone Edu Developer Toolbox or provide its existing clone path. Recommend one
...
including this skill's canonical `.github/skills/toolbox-repository-bridge/SKILL.md` and its
...
This public Toolbox uses standard public package and container registries. Do
not add credential bootstrap files unless a consuming repository explicitly
requires them.
