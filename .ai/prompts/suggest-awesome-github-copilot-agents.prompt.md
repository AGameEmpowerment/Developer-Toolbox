...
2. **Scan Local Custom Agents**: Discover existing custom agent files in `.ai/agents/` folder
...
10. **Download Assets**: For requested agents, automatically download and install individual agents to `.ai/agents/` folder. Do NOT adjust content of the files. Use `#todos` tool to track progress. Prioritize use of `#fetch` tool to download assets, but may use `curl` using `#runInTerminal` tool to ensure all content is retrieved.
...
1. List all `*.agent.md` files in `.ai/agents/` directory
...
- Scan local file system for existing agents in `.ai/agents/` directory
