...
2. **Scan Local Prompts**: Discover existing prompt files in `.ai/prompts/` folder
...
10. **Download Assets**: For requested instructions, automatically download and install individual instructions to `.ai/prompts/` folder. Do NOT adjust content of the files. Use `#todos` tool to track progress. Prioritize use of `#fetch` tool to download assets, but may use `curl` using `#runInTerminal` tool to ensure all content is retrieved.
...
1. List all `*.prompt.md` files directory `.ai/prompts/`.
...
- Scan local file system for existing prompts in `.ai/prompts/` directory
