...
2. **Scan Local Instructions**: Discover existing instruction files in `.ai/instructions/` folder
...
10. **Download Assets**: For requested instructions, automatically download and install individual instructions to `.ai/instructions/` folder. Do NOT adjust content of the files.  Use `#todos` tool to track progress. Prioritize use of `#fetch` tool to download assets, but may use `curl` using `#runInTerminal` tool to ensure all content is retrieved.
...
- **Path-specific instructions**: `.ai/instructions/NAME.instructions.md` (applies to specific file patterns via `applyTo` frontmatter)
